...
public class W3 {
    ...
    public static void main(String[] args) throws InterruptedException, IOException, RestClientException {
        ...
        KStream<String,GenericRecord> stream = builder.stream("W3");
        long expEnd = 60000L;
        stream.map(new KeyValueMapper<String, GenericRecord, KeyValue<? extends String,? extends GenericRecord>>() {

            CSVWriter writer = new CSVWriter(new FileWriter(args[0]+"_W3_output.csv"));
        ...
        typedStreams[0].times(1).followedBy(typedStreams[1].times(1), 60000L)
    ...
    private static Schema loadSchema(final String name) throws IOException {
        try (
                final InputStream input = W3.class
                        .getClassLoader()
