...
public class W2 {
    ...
    public static void main(String[] args) throws InterruptedException, IOException, RestClientException {
        ...
        KStream<String,GenericRecord> stream = builder.stream("W2");
        long expEnd = 60000L;
        stream.map(new KeyValueMapper<String, GenericRecord, KeyValue<? extends String,? extends GenericRecord>>() {

            CSVWriter writer = new CSVWriter(new FileWriter(args[0]+"_W2_output.csv"));
        ...
        typedStreams[0].times(1).every().followedBy(typedStreams[1].times(1), 60000L)
                .to("output_final");
