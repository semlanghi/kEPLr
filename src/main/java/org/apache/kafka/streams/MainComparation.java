        ...
        while (true){
            ...
            streams.metrics().forEach(new BiConsumer<MetricName, Metric>() {
                ...
                public void accept(MetricName metricName, Metric metric) {
                    if(metricName.group().contains("producer") && metricName.description().equals("The number of outgoing bytes sent to all servers per second") )//|| metricName.name().contains("latency"))
