...
public class WrappedKStreamImpl<K,V> extends AbstractStream<K,V> {
    ...
    public WrappedKStreamImpl(KStream<K,V> stream) {
        super((AbstractStream<K, V>) stream);
    }
    ...
    public KTStream<K, V>[] match(final EType<K,V>... eTypes) {
        ...
        for (int i = 0; i < eTypes.length; i++) {
            try {
                ...
                branchChildren[i] = new KTStreamImpl2<K,V>(childNames[i], new CustomTypedKeySerde<K>(keySerde), valSerde, sourceNodes, false, typizationNode, builder, eTypes[i]);
