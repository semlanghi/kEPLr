...
public class ETypeIntAvro extends EType<Integer, GenericRecord> {

    private AvroSchema schema;

    public ETypeIntAvro(Schema schema) {
        this.description = schema.getName();
        this.schema = new AvroSchema(schema);
    }

    private ETypeIntAvro(String description, boolean onEvery, boolean chunkLeft, boolean chunkRight, AvroSchema schema) {
    ...
    @Override
    public EType<Integer, GenericRecord> everyVersion() {
        EType<Integer,GenericRecord> type = new ETypeIntAvro(schema.getAvroSchema());
    ...
    @Override
    public TypedKey<Integer> typed(Integer key) {
        return new TypedKey<>(key,this.description);
    }

    @Override
    public Integer untyped(TypedKey<Integer> typedKey) {
     ...
     */
    @Override
    public EType<Integer, GenericRecord> product(EType<Integer, GenericRecord> otherType, boolean array) {

        if(otherType instanceof ETypeIntAvro) {
            Schema schema = SchemaBuilder.record(this.description + "_X_" + otherType.description).fields()
                    ...
                    .noDefault()
                    .name("y")
                    .type(((ETypeIntAvro) otherType).schema.getAvroSchema())
                    .noDefault()
                    .endRecord();
            return new ETypeIntAvro(schema);
        }else {
    ...
    @Override
    public EType<Integer, GenericRecord> union(EType<Integer, GenericRecord> otherType) {
        Set<EType<Integer,GenericRecord>> temp = new HashSet<>();
        temp.add(this);
        temp.add(otherType);
    ...
    @Override
    public EType<Integer, GenericRecord> clone() throws CloneNotSupportedException {
        return new ETypeIntAvro(description, onEvery, chunkLeft, chunkRight,schema);
    }


    @Override
    public boolean test(Integer key, GenericRecord value) {
    ...
    public boolean equals(Object o) {
        ...
        if (!(o instanceof ETypeIntAvro)) return false;
        if (!super.equals(o)) return false;
        ETypeIntAvro eTypeIntAvro = (ETypeIntAvro) o;
        return Objects.equals(schema, eTypeIntAvro.schema);
    }
